...
namespace ModBuilder.Commands
{
	public class Markdown2Text
	{
		...
		{
			var input = config.GetFile("input");
			var output = config.GetConfig("output", true);

			var mark = new Markdown();
			var text = File.ReadAllText(input);
			var html = mark.Transform(text);
			var result = HTML2Text($"<html>{html}</html>");
			if (output != null)
			{
		...
		static string HTML2Text(string html)
		{
			...
			xsl.Load(new StringReader(transformHtmlToPlaintext));
			var xslt = new XslCompiledTransform(true);
			xslt.Load(xsl, null, null);
			var reader = XmlReader.Create(new StringReader(html));
			...
			xslt.Transform(reader, XmlWriter.Create(writer, settings));
			return writer.ToString();
		}

		static readonly string transformHtmlToPlaintext = @"<?xml version=""1.0"" encoding=""UTF-8""?>
